...
def get_closeness(result, n_result):
	...
	word_thresh = 25 #TODO: Tune this better, and fix this whole distance system

	dist = Levenshtein.distance(text, textn)
	if (dist < word_thresh):	
...
def delete_similar_items(results, sim_items):
...
trim_headers = delete_similar_items(header_results, sim_headers)

#add something here to remove excess dates, random crosshair check
