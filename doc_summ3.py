	...
	for result in results:
		...
		for word in words:
			try:
				...
				if ('.' in s_count):
					sct = s_count.split('.')
					if (len(sct[1]) > dec_place_limit): #TODO: HACK, make better vision!!
						append = True
						for key in string.punctuation:
							if (key == '.'):
								trans_dict[key] = ''.format(key)
								...
								trans_dict[key] = ''.format(key)
						s_count = s_count.translate(str.maketrans(trans_dict))
						count = float(s_count)
				if (append == True):
...
def split_results(results, im_height, im_width):
	d = enchant.Dict("en_US")
	header_divis = 3 #TODO: TUNE THIS
	header_thresh = int(im_width/header_divis) #if startX is in first nth of page, it is a header
	words_real_thresh = 1 #number of real words necessary to be a header, and excluding dates
	...
	return header_results, count_results
...
def find_income_image(pdf_path, income_page_number=0):
	...
	#for now, lets just return the correct one for apple for testing
...
header_results, count_results = split_results(results, orig_height, orig_width)
...
trim_headers = header_results #For now, we are not trimming the headers, we assume the vision is good enough

#TODO: ADD FN HERE TO TRIM UNNECESSARY HEADERS
